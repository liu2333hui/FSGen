...
class Parallel2SerialPrimitive(GeneralPrimitiveModel):	
...
def Parallel2SerialPrimitiveTest( out_features = ['Total_Pwr','Unit_Cycles', 'Energy'], train = 0):
	...
	directory = Path('generated/Parallel2Serial')
	train_sets = list(directory.rglob('1*.txt'))
	...
	if train:
		...
		return ap.execute_train(name = name, 
	 		...
	 		data_features = [
        "bits_out_0",        "sum_toggles_in", "toggles_out_0"],#'toggles_out_0'],	
