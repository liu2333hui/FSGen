...
int sp_prefill_mha_baseline(){
	...
	float KVQGen_dense = 4;
	float KVQGen_sparse_cycles = 3;	
	...
	for(int m = 0; m+mmm < mmm+SMM; m += SN){
				...
				int Q_reuse = (1 <= Qcache & m > 0) |
					...
					float mem_cycles = round;
					//actual_cycles += max(KVQGen_sparse_cycles*1.0, 1.0*mem_cycles);
					actual_cycles += mem_cycles; //no sparsity here unfortunately due to little zeros
	...
	//Power is mostly from the PE array size
	// cout << "LP\tSG\tSB\tSN\tSI\tPE\truntime\t" << endl;
	cout << lp << "\t" << SG  << "\t" <<  SB << "\t" <<  SN   << "\t" <<  SI  << "\t" <<  SG*SB*SN*SI/KVQGen_dense << "\t" << actual_cycles << endl;
	
	// float scale = 1e9/1e12;
	// float TFLOP_MM  = SG*SB*SN*SI;   
	// float TFLOP_SM  = SM_SB*SM_SN*SM_SM;   
...
int prefill_mha_baseline(){
	...
	//Power is mostly from the PE array size
	// cout << "LP\tSG\tSB\tSN\tSI\tPE\truntime\t" << endl;
	cout << lp << "\t" << SG  << "\t" <<  SB << "\t" <<  SN   << "\t" <<  SI  << "\t" <<  SG*SB*SN*SI << "\t" << actual_cycles << endl;
...
PerfResult prefill_mha_fused(
	...
	// cout << "TILE" << (SM*SB*SN*SH)*(SD+SO)<<endl;
	...
	// Wkcache =Wcache; 
		...
		for(int ddd = 0; ddd < DimD; ddd += SDD){
			...
			compute_cycles += KVQGen_sparse_cycles*1.0*sparse_group/group;
		...
		for(int e = 0; e < DimE; e += SE){
			...
			// for(int oo = 0; oo < SO ; oo +=   OO ){
				...
				// #if(DECODE)
					...
					// if(WindowAttenMem){
					// 	int T = 128;
					// 	if(m+mmm >16 and (m +mmm < DimM-T or DimM+T < m+mmm)){
				...
				// }else{
					...
					compute_cycles += fused_sparse_cycles*1.0*sparse_group/group;
					actual_cycles += max(fused_sparse_cycles*1.0*sparse_group/group, 1.0*mem_cycles);
				// }
	...
	// cout << "UTIL\t" << compute_cycles/(float ) actual_cycles << endl;

	float utilization = compute_cycles/(float ) actual_cycles;

	//Hardware mapping to hardware

	PerfResult p;
	p.actual_cycles = actual_cycles;
	p.utilization = utilization;

	// return actual_cycles, utilization;
	return p;
...
SimResult dse_fused(
	...
	//1. high-level params
	design += to_string(H) + "\t" + to_string(HH) + "\t" + to_string(SH) + "\t" + to_string(SSH) + "\t"
		    + to_string(N) + "\t" + to_string(NN) + "\t" + to_string(SN) + "\t" + to_string(SSN) + "\t"
		    + to_string(B) + "\t" + to_string(BB) + "\t" + to_string(SB) + "\t" + to_string(SSB) + "\t"
		    + to_string(M) + "\t" + to_string(MM) + "\t" + to_string(SM) + "\t" + to_string(SSM) + "\t"
		    + to_string(D) + "\t" + to_string(DD) + "\t" + to_string(SD) + "\t" + to_string(SSD) + "\t"
		    + to_string(O) + "\t" + to_string(OO) + "\t" + to_string(SO) + "\t" + to_string(SSO) + "\t"
		    + to_string(PhiSpargAttenTopK) + "\t" + to_string(PhiSpargAttenTopKRatio) + "\t"
		    + to_string(PhiX) + "\t"
		    + to_string(PhiW) + "\t" 
		   
		   + to_string(PhiXMem) + "\t" 
		   + to_string(PhiWMem) + "\t" 
		   + to_string(WindowAttenComp) + "\t" 
		   + to_string(WindowAttenMem) + "\t" ;
	label += "H\tHH\tSH\tSSH\t";
	label += "N\tNN\tSN\tSSN\t";
	label   += "B\tBB\tSB\tSSB\t";
	label   += "M\tMM\tSM\tSSM\t";
	label   += "D\tDD\tSD\tSSD\t";
	label   += "O\tOO\tSO\tSSO\t";
	label += "PhiSpargAttenTopK\tPhiSpargAttenTopKRatio\t";
	label += "PhiX\t";
	label += "PhiW\t"; 
	label += "PhiXMem\t" ;
	label += "PhiWMem\t" ;
	label += "WindowAttenComp\t"; 
	label += "WindowAttenMem\t" ;
	...
	int TMM1  =MM*BB*NN*DD*HH;
	int TSM   =SM_SB*SM_SN*SM_SM;   
	int TMM2  =MM*BB*NN*OO*HH;   
	
	design += to_string(TMM1) + "\t" + to_string(TSM) + "\t" +  to_string(TMM2) + "\t";
	label += "PE_MM1\tPE_SM\tPE_MM2\t";
	...
	//1. Caches
	// label += "MM1\tSM\tMM2\t"
	//We assume the L2 can hold everything or is from DRAM
	// design += to_string(SH*SB*SM*SD) + to_string(SH*SB*SM*SD) + "\t" + to_string(SH*SB*SM*SD) + "\t" +  to_string(SH*SB*SM*SN) + "\t";

	//We assume the L2 can hold everything or is from DRAM
	design += to_string(SH*SB*SM*SD) + "\t" + to_string(SH*SB*SM*SD) + "\t" + to_string(SH*SB*SM*SD) + "\t" +  to_string(SH*SB*SM*SN) + "\t";
	label += "MemK\tMemV\tMemQ\tMemPS\t";
	// int Kcache  = cache;
	// int Vcache  = cache;
	// int Qcache  = cache;
	// int PScache = cache; 	   
	...
	//Assume these two minimal area and power costs
	//2. Memory-group
	// label += "MG\tMG\tMG\t"
	...
	//2. Compute-value
	design += to_string(BB*HH) + "\t"+ to_string(copy) + "\t" + to_string(group) + "\t" +  to_string(multicast) + "\t" +  to_string(ins) + "\t"+  to_string(outs) + "\t";
	label += "Xn\tXc\tXg\tXm\tXi\tXo\t";
	
	design += to_string(BB*HH) + "\t"+ to_string(wcopy) + "\t" + to_string(wgroup) + "\t" +  to_string(wmulticast) + "\t" +  to_string(wins) + "\t"+  to_string(wouts) + "\t";
	label += "Wn\tWc\tWg\tWm\tWi\tWo\t";
	// cout << "copy="<<copy<<"\tgroup="<<group<<"\tmulticast="<<multicast<<"\tins="<<ins<<"\touts="<<outs<<endl;
	// cout << "wcopy="<<wcopy<<"\twgroup="<<wgroup<<"\twmulticast="<<wmulticast<<"\twins="<<wins<<"\twouts="<<wouts<<endl;
			
	
	TN = 1; TNN = 1; TSN = SSM/SM;
	TB = 1; TBB = 1; TSB = SSN/SN;
	TI = 1; TII = 1; TSI = SSD/SD;
	if(SKIP_W && SKIP_X){
		copy = 1;
		group = TNN*TBB/TN/TB;
		multicast = 1;
	...
	//2. Compute-group
	// label += "MM1\tSM\tMM2\t";
	design += to_string(SSB*SSH) +"\t"+ to_string(copy) + "\t" + to_string(group) + "\t" +  to_string(multicast) + "\t" +  to_string(ins) + "\t"+  to_string(outs) + "\t";
			// + wKVQGen_sparse_cycles_avg + wKVQGen_sparse_cycles_worst
	// cout << "copy="<<copy<<"\tgroup="<<group<<"\tmulticast="<<multicast<<"\tins="<<ins<<"\touts="<<outs<<endl;
	// cout << "wcopy="<<wcopy<<"\twgroup="<<wgroup<<"\twmulticast="<<wmulticast<<"\twins="<<wins<<"\twouts="<<wouts<<endl;
		
	label += "GXn\tGXc\tGXg\tGXm\tGXi\tGXo\t";
	
	design += to_string(SSB*SSH) +"\t"+ to_string(wcopy) + "\t" + to_string(wgroup) + "\t" +  to_string(wmulticast) + "\t" +  to_string(wins) + "\t"+  to_string(wouts) + "\t";
	label += "GWn\tGWc\tGWg\tGWm\tGWi\tGWo\t";
	...
	design += to_string(runtime) + "\t"+to_string(utilization)+"\t"+to_string(runtime*1e-9)+"\t"+to_string(mode)+"\t";
	label +=  "actual_cycles\tutilization\truntime\tmode\t";
...
int main(){
	...
	// PhiX = 1;
	// PhiW = 1;
	// PhiSpargAttenTopK  = 1;  PhiSpargAttenTopKRatio = 4;
	// PhiXMem = 1;
	// PhiWMem = 1;
	// WindowAttenMem = 1;	
	...
	// for(PhiX = 0; PhiX </*  */= 1; PhiX++){
	// for(PhiW = 0; PhiW <= 1; PhiW++){
	// for(PhiSpargAttenTopK = 0; PhiSpargAttenTopK <= 1; PhiSpargAttenTopK++){
	// for(PhiXMem = 0; PhiXMem <= 1; PhiXMem++){
	// for(PhiWMem = 0; PhiWMem <= 1; PhiWMem++){
	// for(WindowAttenMem = 0; WindowAttenMem <= 1; WindowAttenMem++){
						
	
	// #if(D1)
	// H = 1;	HH = 1;	SH = 1;	 SSH = 1; 
	// N = 1;	NN = 32;   SN = 16;	 SSN = 4;   SNN = 512;
	// B = 1;  BB = 1;   SB = 1;   SSB = 1;  					
	// M = 1;  MM = 1;   SM = 1;   SSM = 4;   SMM = 512;
	// D = 1;  DD = 64;   SD = 1;   SSD = 1;   SDD = 512;
	// O = 1;  OO = 64;   SO = 1;   SSO = 1; 					
	// #endif
	...
	H = ZH;	HH  = ZHH; 	 SH = ZSH;	 SSH = ZSSH;  					
	N = ZN;	NN  = ZNN;   SN = ZSN;	 SSN = ZSSN;   SNN = 512;
	B = ZB;  BB = ZBB;   SB = ZSB;   SSB = ZSSB;  					
	M = ZM;  MM = ZMM;   SM = ZSM;   SSM = ZSSM;   SMM = 512;
	D = ZD;  DD = ZDD;   SD = ZSD;   SSD = ZSSD;   SDD = 512;
	O = ZO;  OO = ZOO;   SO = ZSO;   SSO = ZSSO; 					
	
						
										
	// for(int i = 0 ; i < 10; i++){
		PerfResult res = prefill_mha_fused(
