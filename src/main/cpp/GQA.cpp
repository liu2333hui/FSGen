...
int prefill_mha_baseline(){
	...
	actual_cycles *= 2;
	actual_cycles += ((DimB*DimM*DimD*DimH)/SG/1/SB/SN)*DRAM_read_cyc*2 ;//+((DimB*DimM*DimD*DimH)/SG/1/SB/SN)* DRAM_read_cyc;
		...
		for(int ddd = 0; ddd < DimD; ddd += SDD){
				...
				for(int e = 0; e < DimE; e +=    SI ){
						int X_reuse = (DimE <= Xcache & d > 0) | (DimE*SMM <= Xcache & h > 0) |
							(DimB*DimE*SMM <= Xcache & ddd > 0) | 
							(DimE <= Xcache & g > 0 );				
						int Wk_reuse = (DimE*SDD*DimG <= Wkcache & m > 0) | 
							(DimE*SDD*DimH *DimG <= Wkcache & b > 0) |(DimE*DimD*DimH*DimG <= Wkcache & mmm > 0) ;					
			#endif
			#if(WS)             // [G,H,D,E]
				for(int h = 0; h < DimH; h +=   1  ){
				for(int d = 0; d+ddd < std::min(DimD,ddd+SDD) ; d +=   SN ){
	...
	// cout << "extra " << ((DimB*DimM*DimD*DimH)/SG/1/SB/SN)*DRAM_read_cyc +((DimB*DimM*DimD*DimH)/SG/1/SB/SN)* DRAM_read_cyc<<endl;
	actual_cycles *= 1;
	actual_cycles += ((DimB*DimM*DimD*DimH)/SG/1/SB/SN)*DRAM_read_cyc*1 +((DimB*DimM*DimD*DimH)/SG/1/SB/SN)* DRAM_read_cyc;
	...
	for(int b = 0; b < DimB; b += SG){
		...
		for(int d = 0; d+ddd < ddd+SDD; d += SI){
				int K_reuse = (SDD*SMM <= Kcache & n > 0) |
					(DimD*DimH*DimB*SMM <= Kcache & nnn > 0) |
					(DimD*DimH*DimB*SMM <= Kcache & g > 0) 
					;
				
				int Q_reuse = (SDD <= Qcache & m > 0) |
					(DimD*DimH*DimB*SMM*DimG <= Qcache & mmm > 0) | 
					 0
					;
		#endif
	...
	for(int ddd = 0; ddd < DimD; ddd += MM_SNN){
		...
		for(int d = 0; d+ddd < std::min(DimD,ddd+MM_SNN) ; d +=   SN ){
					int P_reuse = (1 <= Pcache & d > 0) | 
						(DimB*TiledN*DimH*DimM*DimG <= Pcache & ddd > 0) ;				
...
int prefill_mha_fused(){
	...
	#if(TILE1)
	SH = 16;SN = 64;SB = 4;SM = 1;SO = 1;SD = 1; SG = 1;// SO = 64;
	SNN = 128; SMM = 128; SDD = 128; SOO = 128;
	#endif
	
	#if(TILE2)
	SH = 1;SN = 64;SB = 1;SM = 1;SO = 64;SD = 64; SG = 1;// SO = 64;// SO = 64;
	SNN = 128; SMM = 128; SDD = 128; SOO = 128;
	#endif	

	#if(TILE3)
	SH = 1;SN = 8;SB = 1;SM = 8;SO = 64;SD = 64;SG = 1;// SO = 64;// SO = 64;
	SNN = 64; SMM = 128; SDD = 128; SOO = 128;
	#endif	

	#if(TILE5)
	SH = 4;SN = 4;SB = 4;SM = 4;SO = 8;SD = 8;SG = 2;// SO = 64;// SO = 64;
	SNN = 64; SMM = 128; SDD = 128; SOO = 128;
	...
	#if(TILE4)
	SH = 4;SN = 1;SB = 4;SM = 16;SO = 1;SD = 16;SG = 2;// SO = 64;// SO = 64;
	...
	float scale = 1e9/1e12;
	float TFLOP_MM1  =scale* SM*SB*SN*SD*SH*SG;   
	float TFLOP_SM   =scale* SM_SB*SM_SN*SM_SM;   
	float TFLOP_MM2  =scale* SM*SB*SN*SO*SH*SG;   
		...
		actual_cycles *= 2;
			...
			//For Query
				...
				for(int ddd = 0; ddd < DimD; ddd += SDD){
						...
						for(int e = 0; e < DimE; e +=    SN ){
								int X_reuse = (DimE <= Xcache & d > 0) | (DimE*SMM <= Xcache & h > 0) |
									(DimB*DimE*SMM*DimG <= Xcache & ddd > 0) ;				
								int Wk_reuse = (DimE*SDD <= Wkcache & m > 0) | 
									(DimE*SDD*DimH <= Wkcache & b > 0) |(DimE*DimD*DimH <= Wkcache & mmm > 0) ;					
					#endif
					#if(WS)             // [H,D,E]
						...
						for(int b = 0; b < DimB; b += SB ){
						for(int m = 0; m+mmm < std::min(DimN,mmm+SMM) ; m +=   SM ){
							int X_reuse = (DimE*SMM*DimB*DimG <= Xcache & d > 0) | (DimE*SMM*DimB*DimG <= Xcache & h > 0) |
								(DimB*DimE*SMM*DimG <= Xcache & ddd > 0) ;				
							int Wk_reuse = (1 <= Wkcache & m > 0) | 
								(1 <= Wkcache & b > 0) |(DimE*DimD*DimH <= Wkcache & mmm > 0) ;					
					#endif
	...
	// cout << "extra " << ((DimB*DimM*DimD*DimH)/SG/1/SB/SN)*DRAM_read_cyc +((DimB*DimM*DimD*DimH)/SG/1/SB/SN)* DRAM_read_cyc<<endl;
	// actual_cycles *= 2;
	actual_cycles += ((DimB*DimM*DimD*DimH)/SM/SH/SB/(SO+SD))*DRAM_read_cyc*2 +((DimB*DimM*DimD*DimH)/SM/SH/SB/(SO+SD))* DRAM_read_cyc;
	...
	//Q: [B,N,H,D] m o
		...
		for(int d = 0; d+ddd < std::min(DimD,ddd+SDD); d += SD){
				...
				int Q_reuse = (SDD <= Qcache & m > 0) |
					(SDD*DimH*DimB*SMM*DimG <= Qcache & mmm > 0) |
					
					(SDD*SMM <= Qcache & o > 0) |
					(SDD*DimH*DimB*SMM*DimG <= Qcache & ooo > 0) ;
					
				int P_reuse = (1 <= PScache & d > 0) |
					(DimH*DimB*SNN*SMM*DimG <= PScache & ddd > 0) |
					
					(SMM <= PScache & o > 0) |
					(DimH*DimB*SMM*SNN*DimG <= PScache & ooo > 0) ;				
	#endif
	...
	//P: [B,M,N,H] d o
		...
		for(int o = 0; o+ooo < std::min(DimD, ooo+SOO); o += SO){
			...
			int Q_reuse = (SDD*SNN*DimG <= Qcache & m > 0) |
				(SDD*DimH*DimB*SNN*DimG <= Qcache & mmm > 0) |
				
				(1 <= Qcache & o > 0) |
				(SDD*DimH*DimB*SNN*DimG <= Qcache & ooo > 0) ;

			int P_reuse = (SNN*DimG <= PScache & d > 0) |
				(DimH*DimB*SNN*SMM*DimG <= PScache & ddd > 0) |
