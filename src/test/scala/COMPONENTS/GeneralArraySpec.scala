...
class GeneralSpatialArraySpec(c: GeneralSpatialArray, 
...
object GeneralSpatialArraySpec extends App{ 
		// Testing Params

		val N = 200
		val EDAVerification = false

		val ModuleName = "GeneralSpatialPEArray"//Match class names
			...
			override def driver() = {
					Driver.execute(  Array("--generate-vcd-output" , "on",
					  "--top-name", this.SpecName,
					   ...
					   () => {
							val m = new GeneralSpatialArray(this.HardwareConfig)
							( m  ) 
					   })( c => {
						    val mspec = new GeneralSpatialArraySpec(c, TestVector=this.TestVector, N=this.N,D=this.CLOCK) 
					   	    ( mspec )    
						} ) 
			}
		}
