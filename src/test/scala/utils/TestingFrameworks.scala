...
//	def run(HardwareMap:Map[String,Array[_ >: String with Boolean with Int with Double with Array[Array[String]] with Array[Array[Int]] with Array[Int] with Array[String]  ]], 
	def run(HardwareMap:Map[String,Array[_ >: String with Boolean with Int with Double   ]], 
		...
		for (k <- RuntimeMap.keys){
			...
			idx  = idx + 1
               ...
               var head =  HardwareKeys.reduce(_+"\t"+_) + "\t" + RuntimeKeys.reduce(_+"\t"+_)+"\t"+ TestingKeys.reduce(_+"\t"+_) + "\t"+ Array("Internal_Pwr", "Switch_Pwr", "Leak_Pwr", "Total_Pwr").reduce(_+"\t"+_)+"\t"+ "cycles" +"\t"+ "N" + "\t" + "Area" + "\n"

		if(DontSaveInput){
			head =  HardwareKeys.reduce(_+"\t"+_) + "\t" + RuntimeKeys.reduce(_+"\t"+_)+"\t"+ Array("Internal_Pwr", "Switch_Pwr", "Leak_Pwr", "Total_Pwr").reduce(_+"\t"+_)+"\t"+ "cycles" +"\t"+ "N"+ "\t" + "Area" + "\n"
		}	 
...
val found = files.filter { file => 
  file.getName.endsWith(".v") && !file.getName.endsWith("syn.v")
}
					...
					//New, get the area as well
					val area_report = s"${root}/${module_out_name}/${TopName}.area" //um^2
					val Area = area.get_area(area_report)
					

		  	               var body =  HardwareKeys.map(k => hardware_config(k)).reduce(_+"\t"+_) + "\t" + RuntimeKeys.map(k => runtime_config(k) ).reduce(_+"\t"+_)+"\t"+vectors + "\t"+ avg_power.mkString("\t")+"\t"  + cycles +"\t"+ N+ "\t" + Area + "\n"
					if(DontSaveInput){
		  	               		 body =  HardwareKeys.map(k => hardware_config(k)).reduce(_+"\t"+_) + "\t" + RuntimeKeys.map(k => runtime_config(k) ).reduce(_+"\t"+_)+ "\t"+ avg_power.mkString("\t")+"\t"  + cycles +"\t"+ N+ "\t" + Area+ "\n"
