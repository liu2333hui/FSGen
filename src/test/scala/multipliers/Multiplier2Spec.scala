...
class Multiplier2Spec(c: GenericMultiplier2, 
...
object Multiplier2SpecFromFile extends App{ 
		...
		// Hardware Params
		val HardwareMap = Map(("prec2",Array(parsed("prec2").as[Int] )),
							  ("prec1",Array(parsed("prec1").as[Int] )),
							  ("auto_prec_out", Array(true)),
							  ("radix"   , Array(parsed("radix").as[Int]  )), //2,4,8
							  // ("multiplierType",Array("SimpleMutiplier2")),
							  ("multiplierType", Array(parsed("multiplierType").as[String] )),
							  ("side", Array(parsed("side").as[String])),//"A", "B", "dual"
		...
		// val TestVectorMap:Map[String, Array[Array[Int]]] = Helper.GenSimpleTrainingVectors(mode="bits_no_toggle", p=3, dim=2)
...
object Multiplier2Spec extends App{ 
		// Testing Params
		...
		TestVectorMap = Helper.GenSimpleTrainingVectors(mode="bits", p=8, dim=2)
